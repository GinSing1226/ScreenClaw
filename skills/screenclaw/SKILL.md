...
- 调整参数：查阅常见问题排查章节，确认窗口ID、坐标参数、操作方式等
- 分析截图，精确定位坐标
...
| **Process** | 读取截图 → **质疑窗口选择** → **质疑坐标正确性** → 寻找反例 → 逐个测试 → 根因分析 → 生成反馈 |
...
- 读取执行结果的截图
- **优先质疑窗口选择是否正确**：
  - 子窗口通常是合理选择（大多数操作需要子窗口才能响应）
  - 如果Executor使用主窗口，必须有明确的合理理由
  - 如果子窗口不合理，要求Executor切换到主窗口
- **重点质疑坐标是否正确**：
  - 带着证伪思维，逐一验证Executor报告的坐标
  - **目标坐标**：点击位置是否真的是目标元素？有没有可能是背景、边框、其他元素？
  - **周边元素坐标**：Executor列出的周边元素坐标。逐个检查该坐标是否存在指定元素
  - **驳回规则**：只要发现任何一个坐标（目标或周边）是错的，立即驳回，要求Executor重新分析
- 强制寻找反例（至少3个）：
  - 反例必须与Executor的理由相关联
  - 反例必须在截图中清晰可见
  - 不清晰也是一种反例
...
**证伪流程**：假设可能有误 → **检查窗口选择** → **逐一验证坐标** → 寻找反例（至少3个）→ 逐个测试 → 推翻或确认
...
2. **坐标正确性质疑**：
   - 目标坐标(x,y)是否真的是目标元素？可能是背景、边框、其他元素吗？
   - 周边元素1的坐标是否正确？它是否在截图中该位置？
   - 周边元素2的坐标是否正确？
   - 周边元素3的坐标是否正确？
   - 如果任何坐标不对，Executor需要重新分析截图
...
| **Process** | 读取截图 → **质疑窗口选择** → **质疑坐标正确性** → 根据周边元素定位目标区域 |
| **Output** | 窗口选择评估 + 坐标验证结果 + 目标区域的视觉状态 |
| **Exception** | 图片无法读取 → 要求Executor重新截图 |

**窗口选择质疑**：
- 检查Executor使用的是主窗口还是子窗口
- 如果是主窗口，询问为什么不使用子窗口？是否有合理理由？
- 如果是子窗口，检查截图尺寸是否合理（不是1x1或其他异常尺寸）

**坐标正确性质疑**：
- 逐一检查Executor报告的坐标
- 目标坐标(x,y)是否正确？可能是其他元素吗？
- 每个周边元素的坐标是否在截图中正确显示？
...
| **Process** | 强制寻找≥3个反例：①窗口选择是否合理 ②坐标是否正确 ③周边元素是否准确 ④操作效果是否明显 |
...
| **Trigger** | 反例验证完成 |
| **Input** | 反例列表、验证结果 |
| **Process** | 所有的反例都被推翻？ → 通过：有未推翻的反例 → 不通过，给出根因（特别是坐标或窗口问题） |
| **Output** | 验证结果 + 反馈（明确指出问题坐标或窗口）+ 待办状态更新 |
