...
description: 获取系统可见窗口列表，找到目标窗口的window_id
---
...
### 使用目的和效果
获取系统中所有可见窗口，找到目标窗口的window_id。window_id是后续所有操作（截图、点击、输入等）的必需参数。

建议始终带 `include_children=true` 获取子窗口，部分操作需要子窗口才能响应。

### 适用场景
- 需要操作某个窗口但不知道window_id
- 需要查找特定应用的窗口
- 需要区分主窗口和子窗口
